...
float create_v(float* &vel, unsigned v_max, unsigned size){
    float dv = static_cast<float>(v_max)/static_cast<float>(size);
    vel[0]=0;
...
float *create_v_pow2(unsigned v_max, unsigned size){
    float dv = static_cast<float>(v_max)/static_cast<float>(size);
    float *vel2 = new float[size];
    float v = 0;
    vel2[0] = 0;
    for(unsigned i=1; i<size; i++){
        vel2[i] = (v+dv)*(v+dv);
...
float* create_pdf(float* vel, unsigned size, float T){
    float *pdf = new float[size];
    for(unsigned i=0; i<size; i++) pdf[i] = (exp(-vel[i]*vel[i]/T))/sqrt(T*pi_f);
    return pdf;
}
float rec(float *vel, float *pdf, unsigned first_idx, unsigned size){
...
float mean_rec(float *vel, float *pdf, float dv, unsigned size){
    return 2*dv*rec(vel, pdf, 0, size);
}
float m_near(float *vel, float *pdf, unsigned size, float dv){
...
float m_kahan(float *vel, float *pdf, unsigned size, float dv){
    float sum = 0;
    float t = 0;
    for(unsigned i=0; i<size; i++){
        float y = vel[i]*pdf[i] - t;
        float z = sum +y;
        t = z- sum - y;
        sum =  z;
    }
...
float integral(float *vel, float *pdf, unsigned size, float dv){
    float sum = 0;
    for (unsigned i =0; i<size; i++){
        float m = dv*vel[i]*pdf[i];
        sum+= m;
    }
    return 2*sum;
}
float integral_fma(float *vel, float *pdf, unsigned size, float dv){
    float sum = 0;
    for (unsigned i=0; i<size; i++){
        float m = vel[i]*pdf[i];
        sum = fma(dv, m, sum);
...
unsigned V_max(float T){
    float v = 1;
    while((exp(-v*v/T))/sqrt(T*pi_f)>0) v+=1000;
    while((exp(-v*v/T))/sqrt(T*pi_f)==0) v-=100;
    while((exp(-v*v/T))/sqrt(T*pi_f)>0) v+=10;
    return static_cast<unsigned>(v);
}
int main(){
    float T_arr[]={0.1, 1, 10, 100, 10000};
   ...
   for(unsigned i=0; i<5; i++){
        float T = T_arr[i]; unsigned v_max = V_max_arr[i];
        float *vel_0 = new float[S[0]];
        float dv_0 = create_v(vel_0, v_max, S[0]);
        float *pdf_0 = create_pdf(vel_0, S[0], T);
        float *vel2_0 = create_v_pow2(v_max, S[0]);
        float *vel_1 = new float[S[1]]; float dv_1 = create_v(vel_1, v_max, S[1]); float *pdf_1 = create_pdf(vel_1, S[1], T); float *vel2_1 = create_v_pow2(v_max, S[1]);
        float *vel_2 = new float[S[2]]; float dv_2 = create_v(vel_2, v_max, S[2]); float *pdf_2 = create_pdf(vel_2, S[2], T); float *vel2_2 = create_v_pow2(v_max, S[2]);
        float *vel_3 = new float[S[3]]; float dv_3 = create_v(vel_3, v_max, S[3]); float *pdf_3 = create_pdf(vel_3, S[3], T); float *vel2_3 = create_v_pow2(v_max, S[3]);
        cout<<endl<<"T="<<T<<"\n\n";
        cout<<"Средний модуль скорости для размеров 10, 100, 1000, 6000 соответственно:"<<endl;
        cout<<"Аналитически "<<sqrt(T/pi_f)<<endl;
